...
export function MetricCard({
  ...
  const body = (
    <div
      className={cn(
        "relative flex h-full flex-col overflow-hidden rounded-xl border border-paper-line bg-paper-card p-5 shadow-card transition-colors",
    ...
    >
      ...
      <p className="relative text-label font-semibold uppercase text-ink-faint">{label}</p>
      <p className={cn("relative mt-2 font-display text-[2.75rem] font-bold leading-none tabular-nums", valueTone[tone])}>
        {value}
      </p>
      {subtitle ? <p className="relative mt-1.5 text-xs text-ink-soft">{subtitle}</p> : null}
      ...
      {sparkline && sparkline.length > 1 ? (
        <div className="pointer-events-none relative mt-auto pt-4">
          <AreaChart data={sparkline} height={36} strokeWidth={1.75} sparkline color={sparkColor[tone]} />
        </div>
      ) : null}
    </div>
  );
