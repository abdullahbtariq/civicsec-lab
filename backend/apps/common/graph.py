...
def build_graph(organisation) -> dict[str, list[dict]]:
    ...
    # ------------------------------------------------------------------ #
    open_events = RiskEvent.objects.filter(
        organisation=organisation, status__in=["new", "triaged", "investigating"]
    ...
    # ------------------------------------------------------------------ #
    matches = (
        AssetVulnerabilityMatch.objects.filter(
            asset__organisation=organisation, calculated_risk_score__gte=46
        )
    ...
    # ------------------------------------------------------------------ #
    incidents = (
        Incident.objects.filter(organisation=organisation)
    ...
    # ------------------------------------------------------------------ #
    anomalies = (
        LoginAnomaly.objects.filter(
            organisation=organisation, status__in=["new", "escalated", "reviewed"]
        )
    ...
    # ------------------------------------------------------------------ #
    clusters = (
        NarrativeCluster.objects.filter(
